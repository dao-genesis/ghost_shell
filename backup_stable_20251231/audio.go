...
var (
	audioBroadcast = make(chan []byte, 10) // Reduced buffer for lower latency
	apiMutex       sync.Mutex
...
func runAudioLoop() error {
	...
	// ... (Skipping property store debug code logic)
	...
	// 10ms (100000) - Low latency, frequent small chunks
	...
	for {
		...
		if flags&wca.AUDCLNT_BUFFERFLAGS_SILENT == 0 {
			...
			// 16kHz Mono = ~32 KB/s vs 48kHz Stereo = ~187 KB/s
			...
			// 3. Downmix to MONO for WAN bandwidth
